<?php
/**
 * Environment Loader for PHP 5.3+
 *
 * for CakePHP 2.0+
 * PHP version 5.3+
 *
...
class Environment {
	...
	public static function load($environments = array('development', 'staging', 'production')) {
		foreach ($environments as $env) {
			if (!is_file(static::getEnvDir() . $env)) {
				continue;
			}
	...
	public static function bootstrap() {
		return static::readEnvFile('bootstrap.php');
	}
	...
	public static function readEnvFile($file = 'bootstrap.php') {
		$path = static::getConfigDir() . self::getEnv() . DS . $file;
		if (is_file($path)) {
	...
	public static function getEnvDir() {
		return static::getConfigDir() . 'env' . DS;
	}
